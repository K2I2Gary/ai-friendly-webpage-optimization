# Page is indexed by sitemap, llms.txt, or sitemap.md

> A page that exists but isn't announced anywhere is an orphan, agents can only reach it by crawling links from another page. Orphan pages drop out of RAG indexes, search results, and any incremental re-ingestion that starts from the announced URL set.

`page` | `Discoverability` | `impl 1.0.0` | `discovery.indexed`
