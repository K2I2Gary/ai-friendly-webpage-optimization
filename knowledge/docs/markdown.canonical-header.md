# Markdown mirror sends canonical Link header

> When an agent fetches the markdown mirror, it needs a way to find its way back to the canonical HTML page (for example, to link a citation). The Link header with rel="canonical" is the HTTP equivalent of <link rel="canonical"> for non-HTML responses.

`page` | `Markdown mirror` | `impl 1.0.0` | `markdown.canonical-header`
