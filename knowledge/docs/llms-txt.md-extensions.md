# llms.txt links use .md or .mdx

> Markdown URLs are the format agents can ingest cleanly without parsing HTML or executing JavaScript. Linking to .html pages from llms.txt forces agents to do the heavy lifting and defeats the purpose of having a curated list of agent-friendly URLs.

`site` | `Discoverability` | `impl 1.0.0` | `llms-txt.md-extensions`
...
## How to implement it
For every page you list in llms.txt, link to its markdown mirror rather than its HTML page. If you don’t yet publish markdown mirrors, see markdown.mirror-suffix for how to add them.
